...
// Configuração do ID do cliente
const clientId = process.env.CLIENT_ID || 'botLocal2';
console.log(`Usando clientId: ${clientId}`);

// Caminho da pasta de sessão
const sessionPath = `./.wwebjs_auth/session`;

// ---------------------- Funções Utilitárias ----------------------

// Função para limpar a pasta de sessão (agora usando Promise)
...
async function initializeClient() {
  ...
  client.on('ready', () => {
    ...
    // Keep-alive
    setInterval(() => {
      client.sendMessage('5512981779669@c.us', 'Keep-alive ping')
...
app.get('/', (req, res) => {
  res.send('Servidor está rodando! Chatbot WhatsApp VendasMaisJá.');
});
...
async function start() {
    try {
        // Limpa a pasta de sessão antes de iniciar
        await clearSession();
